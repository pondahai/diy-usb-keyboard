...
//#define SHOW_SCANCODE_MAP_COORDINATE
//#define SHOW_HEX_CODE
//#define SHOW_DEC_CODE

#define TRANSIMITT_BINARY
...
unsigned char old_state[16][8];

void convert_scancode(int i,int j, unsigned char k) {
  #ifdef SHOW_SCANCODE_MAP_COORDINATE
  ...
  #endif
  #ifdef SHOW_DEC_CODE
   Serial.print(String(scancode[i][j],DEC));
   Serial.print(String(' '));
  #endif
  #ifdef SHOW_HEX_CODE
   Serial.print(String(scancode[i][j],HEX));
   Serial.print(String(' '));
   Serial.print(String(k,HEX));
  #endif
  #ifdef TRANSIMITT_BINARY
   Serial.write(0x00);

   Serial.write(scancode[i][j]);

   Serial.write(k);

  #endif
  #if defined(SHOW_DEC_CODE) || defined(SHOW_HEX_CODE)
   Serial.print("\n");
  #endif
}
...
void loop() {
  ...
  for ( i=0 ;i<16; i++){
    ...
    check_now_and_old(i, j++, !digitalRead(A5));
    check_now_and_old(i, j++, !digitalRead(A4));
    check_now_and_old(i, j++, !digitalRead(A3));
    check_now_and_old(i, j++, !digitalRead(A2));
  }
